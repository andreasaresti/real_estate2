...
{
    public function retrieve_from_xml(Request $request)
    {
        ...
        foreach($xml->post as $row)
        {
            ...
            // return true;
            $property_id = '';
            if((string)$row->REAL_HOMES_property_id != ''){
                $property_id = (int)str_replace('est-', "", (string)$row->REAL_HOMES_property_id);
            }
            if($property_id != ''){
                $district = '';
                $municipality = '';
                ...
                $listing_array['property_id'] = $property_id;
                ...
                try {
                    ...
                    if($import_listings){
                        ...
                        foreach($property_type_array as $property_type){
                            $property_types[] = ['property_id' => $property_id, 'name' => $property_type];
                        }
                        ...
                        foreach($images_array as $image){
                            $property_images[] = ['property_id' => $property_id, 'url' => $image];
                        }
                        ...
                        foreach($features_array as $feature){
                            $property_features[] = ['property_id' => $property_id, 'name' => $feature];
                        }
